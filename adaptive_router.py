...
from litellm.router_strategy.adaptive_router.classifiers.classifier_embedder import classify_prompt as classify_category
...
class AdaptiveRouter:
    ...
    async def async_pre_routing_hook(
        ...
        Classifies the request's semantic type (Qwen3-Embedding-0.6B + pgvector
        nearest-neighbor lookup over RequestType categories), then asks the
        Thompson-sampling bandit (`pick_model`) to choose the best model for
        that (request_type, min_quality_tier) combination.
        """
        user_text = (
            get_last_user_message(cast(List[AllMessageValues], messages or [])) or ""
        )

        request_type = classify_category(user_text)
        ...
        verbose_router_logger.debug(
            "AdaptiveRouter[%s]: request_type=%s min_tier=%s -> chose %s",
            self.router_name,
            request_type,
            min_quality_tier,
            chosen_model,
